...
// Handling form submission for the calculator
server.post('/calculate', (req, res) => {
  ...
  // Store operands, operator, and result in session array
  const calculation = { operand1, operator, operand2, result };
  req.session.results = req.session.results || []; // Initialize session array if not exists
  req.session.results.push(calculation); // Store the calculation in the session array
  
  res.redirect('/calculator'); // Redirect back to the calculator page
});
  // A route to render the calculator form
server.get('/calculator', (req, res) => {
  const results = req.session.results || []; // Retrieve session-stored results
  res.render('calculator', { results }); // Render the EJS file and pass session results
});
